...
def new_songs():
    ...
    for n in range(top100_length):
        ...
        top_artists_list.append(top100_artists_names)
    ...
    for n in range(top100_length):
        ...
        top_artists_popularity.append(top100_artists_pop)

    
    print(top_artists_popularity) 

    list_of_artists_n_pop =  list(zip(top_artists_uri_list, top_artists_popularity))

    session["top_artists"] =  top_artists_uri_list

    session["artists_pop"] = top_artists_popularity
    ...
    for i in range(top100_length):
        c_i = top_100_artists["items"][i]["images"][0]["url"]
        top_artists_images.append(c_i)

    print(top_artists_images)
    print(len(top_artists_images))


    dictionary_with_images2 = list(zip(top_artists_list,top_artists_uri_list, top_artists_images ))
    
    return render_template("top_artists_select.html", sorted_array=  dictionary_with_images2)
...
def discover_playlist():
    if request.method == "POST":
        ...
        selected_artists_ids = list(selected_artists_ids)

    #print(selected_artists_ids)

    if not selected_artists_ids:
    ...
    artists_ids = session.get("top_artists")

    
    selected_artists_ids_set = set(selected_artists_ids)
    top_artists_indices = [i for i, item in enumerate(artists_ids) if item in selected_artists_ids_set]

    mean_top_artist_popularity = np.mean([top_artists_popularity[i] for i in top_artists_indices])

    sp = spotipy.Spotify(auth=access_token)

    
    related_artists_ids_list1=[]
    related_artists_pop_list1=[]

    for i in range(len(selected_artists_ids)):

        related_artists = sp.artist_related_artists(selected_artists_ids[i])

        top100_length = len(related_artists["artists"])
        
        for n in range(top100_length):
            related_artists_name = related_artists["artists"][n]["id"]
            related_artists_ids_list1.append(related_artists_name)
        ...
        for n in range(top100_length):
            ...
            related_artists_pop_list1.append(related_artists_name)

        

    related_artists_ids_list2=[]
    related_artists_pop_list2=[]


    for i in range(len(related_artists_ids_list1)):

        related_artists = sp.artist_related_artists(related_artists_ids_list1[i])

        top100_length = len(related_artists["artists"])
        
        for n in range(top100_length):
            related_artists_name = related_artists["artists"][n]["id"]
            related_artists_ids_list2.append(related_artists_name)
        ...
        for n in range(top100_length):
            related_artists_name = related_artists["artists"][n]["popularity"]
            related_artists_pop_list2.append(related_artists_name)


   # print("L1")
    related_artists_ids_list3 = [related_artists_ids_list1, related_artists_ids_list2]
    related_artists_ids_list_flat = [item for sublist in related_artists_ids_list3 for item in sublist]
   # print(related_artists_ids_list_flat)
   # print("L2")
    related_artists_pop_list3 = [related_artists_pop_list1, related_artists_pop_list2]
    related_artists_pop_list_flat = [item for sublist in related_artists_pop_list3 for item in sublist]

    #print(related_artists_pop_list_flat)


    new_treshold = mean_top_artist_popularity * 0.6
    if new_treshold < .6:
        new_threshold = .62
    #print("LEEEEEEEN")
    #print(len(related_artists_pop_list_flat))
    low_pop_indices = [n for n,i in enumerate(related_artists_pop_list_flat) if i< new_treshold]

    low_popularity_pop = [related_artists_pop_list_flat[i] for i in low_pop_indices]

    #print(low_popularity_pop)

    low_popularity_artist_ids = [related_artists_ids_list_flat[i] for i in low_pop_indices]


    print(len(low_popularity_artist_ids))
    low_popularity_artist_ids = list(set(low_popularity_artist_ids))

    print(len(low_popularity_artist_ids))
    low_pop_artists_song_ids=[]
    for i in range(len(low_popularity_artist_ids)):
        ...
        for n in range(len(current_artist_top_songs["tracks"])):
            ...
            low_pop_artists_song_ids.append(low_pop_artists_top_songs_ids)


    if len(low_pop_artists_song_ids) < 50:
    ...
    if len(low_pop_artists_song_ids) > 610:

        random_samp_indices = random.sample(range(len(low_pop_artists_song_ids)), 600)

        session["low_pop_artist_song_ids"] = [low_pop_artists_song_ids[i] for i in random_samp_indices]
    else:
        session["low_pop_artist_song_ids"] = low_pop_artists_song_ids




    low_pop_artists_song_names=[]
    for i in range(len(low_popularity_artist_ids)):
        ...
        for n in range(len(current_artist_top_songs["tracks"])):
            ...
            low_pop_artists_song_names.append(low_pop_artists_top_songs_ids)


    print(len(low_pop_artists_song_names))
    ...
    if len(low_pop_artists_song_ids) > 610:
        ...
        session["low_pop_artist_song_names"] = [low_pop_artists_song_names[i] for i in random_samp_indices]
    else:
        
        session["low_pop_artist_song_names"] = low_pop_artists_song_names
...
def callback_feat_sel_low_pop():
    ...
    if request.method == 'POST':
        if request.form.get('sub') == "Generate Playlist Without Sound Features":
                ...
                if len(ids_best_playlist_1) < 49:
                    print("P1 COND1")
                    best_playlist_1_track_info_list1 = sp.tracks(ids_best_playlist_1)


                    #print("PYTHON FUNCTION")
                    #print(best_playlist_1_track_info_list1)
                    print("LENGTH")
                    print(len(best_playlist_1_track_info_list1["tracks"]))
                ...
                if len(ids_best_playlist_1) > 49 and len(ids_best_playlist_1) < 99:
                    print("P1 COND 2")
                    best_playlist_1_track_info_list1 = sp.tracks(ids_best_playlist_1[0:49])
                    best_playlist_1_track_info_list2 = sp.tracks(ids_best_playlist_1[49:len(ids_best_playlist_1)])
                    ...
                    best_playlist_1_artists1 = []
                    for i in range(len(best_playlist_1_track_info_list2["tracks"])):
                        c_a = best_playlist_1_track_info_list2["tracks"][i]["artists"][0]["name"]
                        print("C_A 1")
                        print(c_a)
                        best_playlist_1_artists1.append(c_a)

                    best_playlist_1_artists.extend(best_playlist_1_artists1)
                    ...
                    best_playlist_1_artists_ids1 = []
                    for i in range(len(best_playlist_1_track_info_list2["tracks"])):
                        c_a = best_playlist_1_track_info_list2["tracks"][i]["id"]
                        best_playlist_1_artists_ids1.append(c_a)

                    best_playlist_1_artists_ids.extend(best_playlist_1_artists_ids1)
                    ...
                    best_playlist_1_images1 = []
                    for i in range(len(best_playlist_1_track_info_list2["tracks"])):
                        c_a = best_playlist_1_track_info_list2["tracks"][i]["album"]["images"][0]["url"]
                        print("JUST TRRACKS INDEX")
                        print(c_a)
                        best_playlist_1_images.append(c_a)

                    best_playlist_1_images.extend(best_playlist_1_images1)


                if len(ids_best_playlist_1) > 99:
                    print("P1 COND 3")
                    indexes_for_selection = random.sample(range(len(ids_best_playlist_1), 99))
                    best_playlist_1_track_info_list1 = sp.tracks(ids_best_playlist_1[indexes_for_selection[0:49]])
                    best_playlist_1_track_info_list2 = sp.tracks(ids_best_playlist_1[indexes_for_selection[49:99]])
                    ...
                    for i in range(len(best_playlist_1_track_info_list2["tracks"])):
                        ...
                        print("JUST TRRACKS INDEX")
                        print(c_a)
                        best_playlist_1_images1.append(c_a)
                ...
                if len(ids_best_playlist_2) < 49:
                    best_playlist_2_track_info_list1 = sp.tracks(ids_best_playlist_2)


                    #print("PYTHON FUNCTION")
                    #print(best_playlist_2_track_info_list1)
                    print("LENGTH")
                    print(len(best_playlist_2_track_info_list1["tracks"]))
                    ...
                    print("PYTHN FUNCTION IDS")
                    print(best_playlist_2_artists_ids)
                ...
                if len(ids_best_playlist_2) > 49 and len(ids_best_playlist_2) < 99:
                    best_playlist_2_track_info_list1 = sp.tracks(ids_best_playlist_2[0:49])
                    best_playlist_2_track_info_list2 = sp.tracks(ids_best_playlist_2[49:len(ids_best_playlist_2)])
                    ...
                    best_playlist_2_artists1 = []
                    for i in range(len(best_playlist_2_track_info_list2["tracks"])):
                        c_a = best_playlist_2_track_info_list2["tracks"][i]["artists"][0]["name"]
                        print("C_A 2")
                        print(c_a)
                        best_playlist_2_artists1.append(c_a)

                    best_playlist_2_artists.extend(best_playlist_2_artists1)
                    ...
                    best_playlist_2_artists_ids1 = []
                    for i in range(len(best_playlist_2_track_info_list2["tracks"])):
                        c_a = best_playlist_2_track_info_list2["tracks"][i]["id"]
                        best_playlist_2_artists_ids1.append(c_a)

                    best_playlist_2_artists_ids.extend(best_playlist_2_artists_ids1)
                    ...
                    best_playlist_2_images1 = []
                    for i in range(len(best_playlist_2_track_info_list2["tracks"])):
                        c_a = best_playlist_2_track_info_list2["tracks"][i]["album"]["images"][0]["url"]
                        print("JUST TRRACKS INDEX")
                        print(c_a)
                        best_playlist_2_images.append(c_a)

                    best_playlist_2_images.extend(best_playlist_2_images1)


                if len(ids_best_playlist_2) > 99:
                    indexes_for_selection = random.sample(range(len(ids_best_playlist_2), 99))
                    best_playlist_2_track_info_list1 = sp.tracks(ids_best_playlist_2[indexes_for_selection[0:49]])
                    best_playlist_2_track_info_list2 = sp.tracks(ids_best_playlist_2[indexes_for_selection[49:99]])
                    ...
                    for i in range(len(best_playlist_2_track_info_list2["tracks"])):
                        ...
                        print("JUST TRRACKS INDEX")
                        print(c_a)
                        best_playlist_2_images1.append(c_a)
                ...
                if len(ids_best_playlist_3) < 49:
                    best_playlist_3_track_info_list1 = sp.tracks(ids_best_playlist_3)


                    #print("PYTHON FUNCTION")
                    #print(best_playlist_3_track_info_list1)
                    print("LENGTH")
                    print(len(best_playlist_3_track_info_list1["tracks"]))
                    ...
                    print("PYTHN FUNCTION IDS")
                    print(best_playlist_3_artists_ids)
                ...
                if len(ids_best_playlist_3) > 49 and len(ids_best_playlist_3) < 99:
                    best_playlist_3_track_info_list1 = sp.tracks(ids_best_playlist_3[0:49])
                    best_playlist_3_track_info_list2 = sp.tracks(ids_best_playlist_3[49:len(ids_best_playlist_3)])
                    ...
                    best_playlist_3_artists1 = []
                    for i in range(len(best_playlist_3_track_info_list2["tracks"])):
                        c_a = best_playlist_3_track_info_list2["tracks"][i]["artists"][0]["name"]
                        print("C_A 3")
                        print(c_a)
                        best_playlist_3_artists1.append(c_a)

                    best_playlist_3_artists.extend(best_playlist_3_artists1)
                    ...
                    best_playlist_3_artists_ids1 = []
                    for i in range(len(best_playlist_3_track_info_list2["tracks"])):
                        c_a = best_playlist_3_track_info_list2["tracks"][i]["id"]
                        best_playlist_3_artists_ids1.append(c_a)

                    best_playlist_3_artists_ids.extend(best_playlist_3_artists_ids1)
                    ...
                    best_playlist_3_images1 = []
                    for i in range(len(best_playlist_3_track_info_list2["tracks"])):
                        c_a = best_playlist_3_track_info_list2["tracks"][i]["album"]["images"][0]["url"]
                        print("JUST TRRACKS INDEX")
                        print(c_a)
                        best_playlist_3_images.append(c_a)

                    best_playlist_3_images.extend(best_playlist_3_images1)


                if len(ids_best_playlist_3) > 99:
                    indexes_for_selection = random.sample(range(len(ids_best_playlist_3), 99))
                    best_playlist_3_track_info_list1 = sp.tracks(ids_best_playlist_3[indexes_for_selection[0:49]])
                    best_playlist_3_track_info_list2 = sp.tracks(ids_best_playlist_3[indexes_for_selection[49:99]])
                   ...
                   # print("PYTHON FUNCTION")
                    ...
                    for i in range(len(best_playlist_3_track_info_list2["tracks"])):
                        ...
                        print("JUST TRRACKS INDEX")
                        print(c_a)
                        best_playlist_3_images1.append(c_a)
...
############################################################
    ...
    track_features_list_complete = list(track_features_list)

    #print(track_features_list_complete)


    all_needed_track_names_flat = list(all_needed_track_names)
...
def playlist_create():
  ...
  #  print(authorization_header)
   ...
   # p1_uris = best_playlist_1_full_list[3]
    ...
    print(selected_playlist_song_uris)
